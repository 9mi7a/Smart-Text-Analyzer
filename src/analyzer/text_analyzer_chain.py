...
class TextAnalyzerChain:
    def __init__(self, provider="gemini", model="gemini-2.5-flash-lite"):
    ...
    def run_core_analysis(self, text: str) -> dict:
        messages = [
            ...
            {"role": "user", "content": self.core_prompt},
            {"role": "user", "content": f"TEXT:\n{text}"},
        ]

        return self.safe_parse_json(self.llm.chat(messages))

    def run_bias_analysis(self, text: str) -> dict:
        messages = [
            {"role": "user", "content": self.system_prompt},
            {"role": "user", "content": self.bias_prompt},
            {"role": "user", "content": f"TEXT:\n{text}"},
        ]
